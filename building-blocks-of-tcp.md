...
> 1. $&gt; sysctl net.ipv4.tcp\_window\_scaling  
> 2. $&gt; sysctl -w net.ipv4.tcp\_window\_scaling=1
...
_拥塞窗口大小\(cwnd\)_ 发送方在没有收到客户端 ACK 之前处在传输状态的最大数据量。
...
> 1. $&gt; sysctl net.ipv4.tcp\_slow\_start\_after\_idle  
> 2.  sysctl -w net.ipv4.tcp\_slow\_start\_after\_idle=0
...
_带宽延迟成绩\(BDP\)_   数据线路容量和点到点延迟的乘积。结果是在任意时间最大在途的未接受数据量。

无论是发送方还是接收方经常性地被迫停下来等待之前数据包的 ACK 会造成数据流的间隙（图 2-7\)，这会限制连接的最大吞吐量。

![](assets/2017-04-21-20-03-24.png)

图 2-7 由于过小的拥塞窗口造成的传输间隙

为了解决这个问题，窗口大小通常要调的足够大，两方均可以持续接受数据直到收到之前数据包 ACK 到达，这样便没有间隙，达到了最高吞吐量。因此，最优的窗口大小取决于往返时间！选一个较小的窗口大小，你会限制连接的吞吐量，两者之间的无论可用或者建议带宽是多少。

那么 rwnd 和 cwnd 的值应该是多大呢？实际计算一个简单的例子。让我们建设最小的 cwnd 和 rwnd 大小是 16 KB，往返时间是 100 ms:  
$$16 KB = \(16\_1024\_8$\) = 131,072bits$  
$$/frac{131,072bits}{0.1s}=1,310,720 bits/s$$  
$$1,310,720 bits/s = /frac{1,310,720}{1,000,000}=1.31Mbps $$  
无论接收方和发送方之间的可用带宽是多少，TCP 连接的吞吐量不会超过 1.31 Mbps！为了达到更高的吞吐量，我们需要提升窗口大小或者降低往返延迟。

类似的，当我们知道了往返时间和两端的可用带宽，我们可以计算最优的窗口大小。在这个场景中，让我们假设往返时间是 100 ms，但是发送方有 10 Mbps 的可用带宽，接收方有 100 Mbps 的高吞吐线路。假设两者之间没有网络拥塞，我们的目标是充分利用到客户端的 10 Mbps 的线路:  
$$10 Mbps = 10 * 1,000,000 = 10,000,000 bits/s$$  
$$10,000,000 bits/s = /frac{10,000,000}{8 * 1024} = 1,221 KB/s$$  
$$1,221 KB/s * 0.1s = 122.1 KB$$

窗口大小最少需要到 122.1 KB 才能最大化 10 Mbps 的线路。回顾一下 64 Kb 的最大接收窗口在没有窗口缩放的情况下是不够用的，查看[窗口缩放\(RFC 1323\)](https://hpbn.co/building-blocks-of-tcp/#window-scaling-rfc-1323)，仔细检查一下的客户端和服务器的设置！

好消息是，窗口大小的协商和调整是由网络协议栈自动管理并进行相应的调整。坏消息是，有时它仍然是 TCP 性能的影响因素。如果你曾经想要弄清楚为什么你的连接只能用可用带宽的一部分传输数据，很可能是因为数据窗口过小，网络环境过差，丢包让拥塞窗口重置，或者是网络结构限制了网络连接的速率。

> **高速局域网的带宽延迟乘积**
>
> BDP 是往返时间和目标数据速率的成绩。尽管当往返时间通常是高延迟网络中的瓶颈，他也可以成为局域网中的瓶颈!
>
> 为了达到 1 ms 往返时间下 1 Gbit/s 的速率，我们至少需要 122 KB 的拥塞窗口。这个计算和我们之前的计算相似，我们简单的给目标速率添加了几个零并且减少了往返延迟的几个零。

# 线头阻塞

TCP 提供了一个在不可靠信道上建立一个可靠网络的抽象，这包含了基本的包错误检查和纠正，顺序传递，丢包重发等，同时也包括了设计用来提升网络效率的流量控制，拥塞控制和拥塞避免。将这些特点组合起来是的 TCP 成为了大多数应用的首选。

尽管 TCP 是最常用的选择，但不是唯一的选择，或者说并不是所有情况下的最优选择。具体来说，如顺序且可靠的数据包数据包传递这些特点并不总是必要的但是却造成了不必要的延迟和负面的性能影响。

为了理解这些情况，我们回顾一下每一个 TCP 数据包都有一个序列号，并且数据必须按照顺序送达接收方，如图 2-8 所示，这简化了应用层的代码。

![2-8](assets/2017-04-22-16-47-25.png)

图 2-8 TCP 中的线头阻塞

然而这是通过引入数据包到达时间的不可预测的延迟因素完成的，通常称之为抖动\(Jitter\)，这对应用性能是负面影响。

一些应用可能并不需要可靠或者顺序传递数据包，如果每一个数据包都是单独的信息，那么顺序传递严格来说是没有必要的，如果每一个数据包都覆盖了之前的所有信息，那么就完全不需要可靠传递。不幸的是，TCP 协议并不提供这样的配置，所有的数据包是有序的，且按顺序传递。

应用层可以处理无序送达或者数据包丢失的情况，并且它们对延迟或者抖动是敏感的话，最好选择其他的协议，比如说 UDP。

> **丢包是正常的**
>
> 实际上，数据包丢失是达到 TCP 协议最高性能的必要条件！一个丢失的数据充当了反馈机制的医院，这允许接收方或者发送方调整他们的发送速率用于避免使网络过载，并且最小化延迟，查看[本地路由器的缓存过满](https://hpbn.co/primer-on-latency-and-bandwidth/#bufferbloat-in-your-local-router)。更重要的是一些应用容忍丢包而不受负面影响，音频，视频和游戏状态更新等常见的应用数据不需要可靠或者顺序传递的应用，这也是为什么 WebRTC 使用 UDP 作为底层协议。
>
> 如果一个数据包丢失了，那么音频编码可以简单的茶语一个短暂的间隔并且处理接下来收到的数据包。如果这个间隔很小，用户可能注意不到，而且等到丢失的数据需要承担引入多变的输出暂停的因素，这可能造成更差的用户体验。
>
> 同样的，如果我们更新游戏状态，收到 T 时刻的数据包后，等待 T-1 时刻的数据包是没有意义的。理想情况下，我们在收到每一个数据包后更新数据，但是为了避免游戏延迟，我们可以接受间隙的丢包。

# TCP 协议优化

TCP 是一个设计用来适应大多数网络连接并且充分利用底层网络。因此，优化 TCP 协议最好的方式是让 TCP 感知网络条件并针对上层和下层的需求调整自身的行为，无线网络可能需要不同的拥塞控制算法，一些应用可能需要自定义服务质量\(QoS\)的语义来传递最好的体验。

不同的应用需求间的相互作用以及众多 TCP 算法间的相互结合是的 TCP 的调整和优化是学术界和工业界永无止境的研究领域。在这一章中，我们肤浅的了解了一下影响 TCP 性能的几个因素。附加的一些机制如选择性确认（SACK），确认延迟，快速重传等等，使得 TCP 会话变得十分复杂\(或者说有趣，看你怎么理解\)。

尽管算法的细节和反馈机制会不断更新，核心原则和它们的含义是不会改变的：  
1. TCP 三次握手引入了一整个往返时间的延迟  
2. TCP 慢启动应用在每一个新建立的连接上  
3. TCP 流和拥塞控制条件所有连接的吞吐量  
4. TCP 吞吐由当前拥塞窗口大小调节

结论是在现代的高速网络下， TCP 连接的速率通常有接收方和发送方直接的延迟决定的。尽管带宽在不断增长，延迟受限于光速，并且已经十分接近最大值。在大多数情况下，延迟而不是带宽是 TCP 的瓶颈，如如 2-5 所示。

# 调节服务器设置

在调节 TCP 中具体的缓冲区大小和超时变量之前，你最好先升级主机系统到最新的版本。TCP 最佳实践和底层算法让他的性能不断得到提升，大多数的变化只能在最新的内核中使用。简单来说，保存你的服务器最新以确保发送端和接收端的 TCP 连接的交流最优。

_note_

> 升级系统内核版本看起来是一个很简单的建议。然而实际上会遇到很多抵抗。许多现存的服务器是按照特定的内核版本设定的，一个系统管理员不愿意升级。
>
> 每一个升级带来了它的风险，但是也得到了最好的 TCP 性能，这像是一个你可以进行的投资。

在使用了最新的内核之后，最佳的实践是确保你的服务器按照如下配置：

_增大初始拥塞窗口_  
一个更大的初始拥塞窗口运行 TCP 在第一次往返中传输更多的数据，有效加速窗口增长。

_慢启动重置_  
禁用慢启动可以提升周期性发送数据的 TCP 长连接的性能

_窗口缩放 \(RFC 1323\)_  
启用窗口缩放增大了最大接受窗口，运行高延迟网络达到更好的性能。

_TCP 快速建立_  
允许初始 SYN 包在特定情况下携带应用数据。TFO 是一个新的优化，这需要客户端和服务器双方的支持，研究一下你的应用是否可以使用。

在最新的内核中将上述设置组合起来可以实现最好的性能，每一个独立的 TCP 连接都可以延迟低和带宽高。

_note_

> 对于 Linux 用户，ss 是一个很有用的工具，显示已经打开的套接字连接的统计信息。在命令行中运行 ss --options --extended --memory --processes --info 观察每一个连接和各自的连接设置

# 配置用户行为

调整 TCP 的性能可以为服务端和客户端的每一个连接提供最佳的吞吐量和延迟。然而，应用程序如何使用每一个新的或者已建立TCP连接对性能也有很大的影响：  
1. 没有比特比不发送更快；发送更少的比特  
2. 我们不能让比特传输的更快，但是我们可以让它们更近。  
3. TCP 连接重用对性能有关键的影响

减少不必要的数据传输当然是最简单有效的方法。例如，减少不必要的资源，或者通过适当的压缩算法减少数据量。其次，我们可以在距客户端更近的地方部署服务器，比如使用 CDN， 将有减少往返延迟，并显着提高 TCP 的性能。最后，尽可能的重用现有的 TCP 连接来最大限度地减少慢启动和拥塞控制机制所带来的开销。

# 性能检查列表

优化 TCP 连接无论对哪种应用，哪种接入到服务器的连接来说，都是至关重要的。一个简短的检查列表：  
1. 升级到最新版本的服务器内核  
2. 确保拥塞窗口大小设置为10  
3. 确保窗口缩放启用  
4. 禁用闲置后慢启动  
5. 消除冗余数据传输  
6. 压缩传输数据  
7. 在离用户更近的地方部署服务器，减少往返时间  
8. 尽可能重用 TCP 连接  
9. 研究 [为 HTTP 设置 TCP](https://hpbn.co/http-tcp) 的推荐
