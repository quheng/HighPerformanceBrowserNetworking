...
![3-1](assets/2017-04-24-11-07-44.png)图 3-1 IPv4 头\(20 字节\)
...
![3-2](assets/2017-04-27-14-30-58.png)图 3-2 UDP 头
...
_不保证信息送达_ 没有回应，重发或者超时

_不保证按序送达_ 没有包序号，没有重排序，没有线头阻塞

_没有连接状态跟踪_ 没有连接建立或者关闭的状态机

_没有拥塞控制_ 没有内键的客户端或者网络反馈机制
...
![3-4](assets/2017-04-25-14-13-30.png)图 3-4 丢弃不匹配的数据包
...
2. 一旦协商完成，两端通过向 TURN 服务器发送数据进行通信，TURN 服务器将数据中继给另外一端

当然，这种交换的明显弊端是数据交换不再是端到端的了！TURN 是一个最可靠在任何网络中提供连接的方式，但是它给 TURN 服务器带来了很大的负担，至少需要有足够的带宽给所有的数据流。结果是，TURN 是直接连接失败的最好的备选方案。

> **STUN 和 TURN 实践**
>
> 谷歌的 ligjingle 是一个开源的 C++ 库用来建立端到端的连接，它在底层实现了STUN，TURN，ICE等。这个库用来实现 Google Talk 聊天应用，文档给 STUN 与 TURN 在现实世界中的性能提供了有价值的参考  
> 1. 92% 的情况下可以直接建立连接\(STUN\)  
> 2. 8% 的情况需要中继\(TURN\)
>
> 不幸的是，甚至是 STUN，许多用户无法直接建立端到端的通道。为了提供一个可靠的服务，我们还是需要 TURN 中继，这为无法建立直接端到端连接时提供了一个备用方案。

建立一个高效的 NAT 穿透方案不是一件简单的事情。幸好我们可以借助 RFC 5245 提出的交互式连接建立协议\(ICE\)来帮助我们完成这一任务。如图 3-7 所示，ICE 是一个用来寻找端与端之间最高效隧道建立方法的协议和一组方法。如果可能的话建立直接连接，或者通过 STUN 协商，如果都失败了则采取 TURN。

![3-7](assets/2017-04-26-16-10-52.png)

图 3-7 ICE 尝试直接连接，STUN 和 TURN

实际上，如果你通过 UDP 建立 P2P 应用，那么你最好利用现有的平台 API 或实现了 ICE, STUN 和 ICE 的第三方库。现在你应该了解了这些协议，并且可以去安装、配置相应的方案了。

# UDP 优化

UDP 是一个简单的协议并且经常用来启动新的传输协议。实际上，UDP 主要的特性就是它忽略了很多特性，没有连接状态，握手，重传，重组，重排序，拥塞控制，拥塞避免，流量控制甚至是错误检查。然而，这些灵活度对于面向信息的传输层的实现者而言也是一个负担。你的应用需要重新实现很多的功能，并且需要另一端支持该协议。

TCP 有内建的流量与拥塞控制以及拥塞避免，但是 UDP 应用必须自己实现这些机制。UDP 对于拥塞是不敏感的，它很容易造成网络过载，导致网络性能下降甚至是网络拥塞拥塞崩溃。

如果你想要为你自己的应用配置 UDP，请确定阅读并研究了当前的最佳实践和推荐。RFC 5405 提供了应用程序通过单播 UDP 传送数据设计指南。下面是一个简短的例子：
...
4. 应用应该使用和 TCP 相似的带宽
5. 应用应该可以在丢包之后重传
6. 应用不应该发送超过 MTU 的数据报
7. 应用应该处理数据报丢失，重复和重排序
8. 应用应该可以支持两分钟的延迟
9. 应用应该开启 IPv4 UDP 校验和，必须开启 IPv6 校验和
10. 应用可能使用长连接（最小 15s 的间隔）

设计一个新的传输协议需要仔细思考，计划和调研。如果可能的话，使用现有的库或者框架来实现 NAT 穿透，使其能够与其他来源的网络建立通信。好消息是 [WebRTC](https://hpbn.co/webrtc/) 就是这样的框架。
