...
表面看来，我们的数字字母表（0 和 1）需要被翻译到模拟信号（无线电波）。*调制* 是数字到模拟转换的过程，不同的 “调制字母表” 需要不同的数字信号编码算法，这导致了效率的不同。字母表和符号速率的组合决定了最终的信道吞吐量。用一个例子来说明：
1. 接收方和发送方可以处理 1,000 个脉冲或者符号每秒 （1,000 波特）
2. 传输么承恩符号代表了一个不同的比特序列，由字母表决定（例如：2-比特 字母表：00，01，10，11）
