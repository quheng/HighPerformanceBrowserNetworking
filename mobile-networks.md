...
首先，假设用户已经通过 4G 网络进行身份验证，并且移动设备空闲。接下来，用户键入URL 并点击 “Go” 。那么接下来会发生什么？

首先，由于电话处于空闲 RRC 状态，天线必须与附近的无线电塔同步，并发送一个建立新无线电上下文的请求（图7-12，步骤1） - 这个协商需要往返手机和无线电塔之间的几个来回，可能需要 100 毫秒。对于由服务网关管理 RRC 的早期网络，这一协商过程延迟远高于几秒钟。

一旦无线电上下文建立，设备从无线电塔获取所需资源，并且能够以指定的速率和信号功率发送数据（步骤2）。将数据包从用户传输到无线电塔的时间被称为“用户面单向延迟”，4G 网络最多需要 5 毫秒。因此，执行 RRC 转换导致了第一个数据包的延迟大大增加，只要无线电处于高功率状态，之后的数据包的第一跳延迟便会是一个固定值。

![7-12](assets/2017-08-02-23-51-55.png)

图 7-12 LTE 请求流程的延迟

但是，到这里还没有结束，因为我们只是将数据包从设备发送到无线电塔！从无线电塔，数据包必须通过核心网络，穿过 SGW 到 PGW（步骤3），最终到达公共互联网（步骤4）。不幸的是，4G 标准没有保证这条路径的延迟，因此不同的运营商这段延迟不同。

**note**
>实际上，一旦设备处于连接状态（即没有由初始数据包引起的控制面延迟），许多 4G 网络的端到端延迟往往在 30-100ms 的范围内。因此，如果第一个无线跳跃大约需要 5 毫秒，那么运营商核心网络内的路由和传输大约需要 25-95 毫秒。

接下来，假设浏览器已经获取了请求的页面，并且用户正在浏览内容。天线已经空闲了几十秒，这意味为了节省电池并释放其他用户的网络资源， RRC 可能将用户转移到 DRX 状态[LTE RRC 状态机](https://hpbn.co/mobile-networks/#lte-rrc-state-machine)。此时，用户决定访问浏览器中其他地址，从而触发新的请求。现在发生会什么？

几乎是重复我们刚刚看到的工作流程，除了因为设备处于休眠（DRX）状态，稍稍加快了协商过程（图7-12，步骤1），休眠到连接的状态转换大约需要 50毫秒（表7-9）。

总而言之，用户发送新请求的会产生几个不同的延迟：

*控制面延迟*

RRC 协商和状态转换引起的固定的一次性延迟成本：空闲到活动时小于 100ms，休眠到活动时小于 50ms。

*用户面延迟*

在设备和无线电塔之间传输的应用数据包的固定成本：<5 ms。

*核心网络延迟*

将数据包从无线电塔发送到包网关的成本，不同运营商之间不同：实际中大约需要 30-100毫秒。

*互联网路由延迟*

运营商的包网关与公网上的目的地址之间可变的延迟成本。

前两个延迟受到 4G 标准的限制，核心网络延迟因运营商而异，最后一个延迟您可以通过战略性地将服务器距客户更近来降低;参见前面关于[光速和传播延迟](https://hpbn.co/primer-on-latency-and-bandwidth/#speed-of-light-and-propagation-latency)的讨论。

>**移动网络中的延迟和抖动**
>关于移动网络的一个常见的问题是数据包延迟的变化或抖动。确实有许多有用的组件可以影响延迟。然而，一旦您考虑了第一个数据包发生的 RRC 状态转换的控制面成本，您可能会发现，性能实际上比您预期的更加可预测。
>
>在 LTE 中，控制面开销达到 100 毫秒。使用LTE-Advanced，这个数字进一步降低到 50 毫秒。然而，在早一代的网络中，这种协商可能需要几秒钟！
>
>核心网络路由延迟是移动网络中总体延迟中的第二个，通常影响很大。核心网络中具体的延迟因网络版本以及运营商部署的具体基础设施而异。然而，虽然很少的运营商公开宣传其延迟性能，也许是因为没有什么值得骄傲的，通常可以在他们的技术常见问题中找到。
>
>例如，AT&T 是美国最大的移动供应商，它为网络中各版本的核心网络延迟有如下预期，这也是整个行业的典型值：

版本|LTE|HSPA+|HSPA|EDGE|GPRS
--|--|--|--|--|--
延迟|40–50 ms|100–200 ms|150–400 ms|600–750 ms|600–750 ms

表 7-10  AT&T 部署好的 2G 到 4G 网络的延迟

>相比之下，赤道地球周长为 24901 公里，光线行驶需要 133.7 毫秒。换句话说，将大多数移动请求视为平均一次环球旅行并不是完全不合理！


## 入站数据流
现在来看看相反的情况：用户的设备是空闲的，但数据包必须从 PGW 路由发送到用户（图7-13）。再一次请记住，所有连接都终止于 PGW，这意味着设备可以在其天线关闭的情况下处于空闲状态，但是设备之前建立的连接（例如 TCP 长会话）仍然可以在 PGW 处于活动状态。

![7-13](assets/2017-08-03-17-04-09.png)

图 7-13 LTE 入站数据流延迟

如前所述，PGW 将入站数据包发送到 SGW（步骤1），SGW 查询 MME。然而由于多个无线电塔组合形成了“跟踪区”，MME 可能不知道当前确切的为用户服务的无线电塔。每当用户进入不同的跟踪区时，MME它的位置位置信息，但在相同的跟踪区内的切换无线电塔不会触发 MME 的更新。

相反，如果设备空闲，则 MME 向跟踪区中的所有无线电塔发送寻呼消息（步骤2），每个塔依次在共享无线信道上广播通知（步骤3），指示设备为了接收入站数据应重新建立无线电上下文。设备周期性地唤醒以监听寻呼消息，如果发现自己在寻呼列表上，则它与无线电塔之间进行协商（步骤4），重新建立无线电上下文。

一旦无线上下文建立完毕，执行协商的无线电塔塔向 MME 发送消息（步骤5），指示用户在哪里，并由 MME 发送给服务网关，网关最终发送数据包（步骤6 ）到无线电塔，最终将数据包传递（步骤7）到设备！

一旦设备处于连接状态，则在无线电塔和服务网关之间建立直接隧道，这意味着进一步入站数据包直接发送到到无线电塔而不用寻呼开销，跳过了步骤 2-5。再次说明，第一个数据包在移动网络上的延迟更高！这需要好好计划一下。

**note**
>上述数据包流对 IP 及以上的所有层（包括我们的应用程序）都是透明的：每一级的 PGW，SGW 和 eNodeB 都会缓存数据包，直到将数据包发送到设备。实际上，控制面协商导致的第一个数据包延迟最高意味着数据包到达时间有着可被观测到的延迟抖动。


# 异构网络（HetNets）
现有的 4G 无线电和调制技术已经到达了现有无线信道的理论的极限。因此，无线性能下一次数量级级别的性能提升不会来自无线接口的改进，而是由于无线网络更加智能的拓扑结构，特别是通过广泛部署多层异构网络（HetNets），这将需要改进许多单元内配合，切换和干扰管理。

HetNets 背后的核心思想十分简单：使用小单元格覆盖整个区域而不是一个大的单元格。大单元格内会产生很多的用户竞争，小单元格使得路径损失最小，传输功率更低，所有用户的性能更好。

![7-14](assets/2017-08-03-17-47-10.png)

图 7-14 异构网络信息图(Ericsson)

单个大单元可以在低密度无线环境中覆盖多达几十平方英里，但实际上在高密度城市和办公环境中，可以限制在仅仅能 50 到 300 米内的区域！换句话说，它只能可以覆盖小区域或者几个建筑物。相比之下，微单元设计用来为覆盖一个特定的建筑物，picocell 可以为一个或多个单独的楼层提供服务，femtocell 可以覆盖一个小公寓，并利用现有的宽带服务作为无线回程。

然而，请注意 HetNets 不是简单地用许多小单元替换大单元。相反，HetNets 将多个单元格重叠在一起！通过部署重叠的无线网络层，HetNets 可以为所有用户提供更好的网络容量和更大覆盖范围。然而，如何减少干扰，提供足够的上行链路容量，以及创建和改进各种网络层之间无缝切换的协议成为了突出的挑战。

这对构建移动应用的开发人员意味着什么？不同单元之间的切换次数会显着增加并需要作出相应的调整：延迟和吞吐量可能会有很大差异。

> **建模与管理无线网络容量**
>Picocell 通常被移动运营商用来将覆盖范围扩展到信号质量可能较差的室内和室外区域，或者在电话使用非常密集的地区增加网络容量，例如，大型公共区域，会议厅，体育馆，火车站，等等。一些 picocell 可能会被永久部署，而另一些 picocell 可能会被放置在特定的场合：无线网络容量的规划和建模（图7-15）既是艺术又是科学！
>
>诸如前面所示的名为 TamoGraph Site Survey 专用建模软件通常用于建模物理环境，活动用户数量和正在使用的无线技术（上一个例子中的 WiFi ）来帮助确定所需的数量，位置和网络配置。

![7-15](assets/2017-08-03-19-34-33.png)

图 7-15 无线容量规划与 TamoGraph

# 真实世界中 3G，4G 和 WiFi 性能
由于 3G 或 4G 网络所引入的额外协议，网关和协商机制增加复杂性，人们不得不怀疑它们是否值得引入。相比之下，WiFi 的实现要简单得多，似乎也工作得很好，不是吗？回答这个问题需要很多说明，测量无线性能受到数十种环境和技术因素的影响。此外，所使用的评估标准也会影响到结果：
...
然而，虽然几十个不同的利益相关者（例如用户，运营商和手机制造商）都有自己的优先级列表，但是 4G 网络早期的测试显示出非常有前途的结果。事实上，诸如网络延迟，吞吐量和网络容量等关键指标往往超过 WiFi！

一个具体的例子是，密西根大学和 AT&T 实验室之间的联合研究项目在美国进行了全国范围的测试（图7-16），比较了 4G，3G 和 WiFi（802.11g，2.4GHz）的性能：

1. 针对 46 个不同的测试实验室节点进行性能测试，该节点是互联网测试工具的开放平台，并通过 MobiPerf 开源测试客户端进行测试。
2. 2011 年下半年两个月的测试中，已有 3300 名用户进行了测试。

![7-16](assets/2017-08-03-20-13-37.png)

图 7-16  WiFi, LTE, 与 3G 性能的测试分析结果

**note**
>箱线图将每种连接类型的大量有用的信息封装到一个小图形中：线条显示整个分布的范围，箱体显示 25％-75％ 分位数，黑色水平线是中位数。

当然，一个单一的测试并不能提供普适的规则，特别是在性能方面，但是结果仍然非常有意义：早期 LTE 网络显示出巨大的网络吞吐量性能，更令人惊讶的是与其他无线标准相比更稳定的 RTT 和数据包抖动与延迟。

换句话说，至少在这个测试方面，LTE 比 WiFi 是可以相提并论的，甚至性能更好，这也表明改进性能是可能的，所有额外的复杂性都是有回报的！移动网络不一定是缓慢的。事实上，我们有理由相信我们可以并且会使之更快。

**note**
>有关 4G 的性能研究，分析和结论的全部内容可以请参见 MobiSys 在 2012 年提出的“4G LTE 网络性能和功耗特性的深入研究”。
