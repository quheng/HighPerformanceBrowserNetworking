...
## 延迟是性能的瓶颈
我们已经在前面的章节中涵盖了所有必要的主题，以便提供一个很好的定性理论来解释为何延迟是日常网页浏览的瓶颈。不过，图片胜过千言万语，所以让我们来看看作为 SPDY 协议的创建者之一的 Mike Belshe 做出的定量研究结果（图10-6），不同带宽和延迟对 Web 上某些最受欢迎的页面加载时间的影响。

![10-6](assets/2017-08-12-18-18-46.png)

图 10-6 不同延迟和带宽下的页面加载时间

>*note*
>
>Mike Belshe 的这项研究成为 Google 开发 SPDY 协议的起点，后者成为 HTTP/2 协议的基础。

在第一次测试中，连接延迟保持固定，连接带宽从 1Mbps 增加到 10 Mbps。请注意，将连接从 1 升级到 2Mbps 时页面加载时间几乎减半 - 完全是我们预期的结果。然而随之而来的是，每次带宽的提高带来的收益递减。在可用带宽超过 5Mbps 后继续提升到 10 Mbp，页面加载时间只提高了 5％！

Akamai 的宽带速度报告（[接入网的带宽](https://hpbn.co/primer-on-latency-and-bandwidth/#bandwidth-at-the-network-edge)）显示，美国的一般消费者访问网络的带宽已经达到了 5 Mbps，许其他多国家迅速接近或已经超过了这一数字。我们得出的结论是，如果想要提高网络浏览速度，美国普通消费者将不会因为提升连接的可用带宽而受益。她可能提升数据流的速度或上传较大的媒体文件，但是包含这些文件的页面的加载速度不会变快：带宽并不重要。

然而，延迟实验讲述了一个完全不同的故事：延迟每改进 20 毫秒，页面加载时间线性提高！也许 ISP 应该选择优化延迟，而不仅仅是带宽？

>为了加快互联网的发展速度，我们应该寻找更多的方法来减少 RTT。如果我们可以将跨越大西洋的 RTT 从 150ms 减少到 100ms 会发生什么？这会对互联网的速度产生更大的影响，而不是将用户的带宽从 3.9Mbps 提高到 10Mbps 甚至 1Gbps。
>
>减少页面加载时间的另一种方法是减少页面加载所需的往返次数。如今，网页在客户端和服务器之间需要一定数量的往返。往返次数主要是由启动客户端和服务器之间进行通信的握手造成的（例如，DNS，TCP，HTTP），以及由通信协议引起的往返（例如 TCP 慢启动）。如果我们改进协议使之可以通过更少的往返次数来传输这些数据，那么我们也应该能够改进页面加载时间。这是 SPDY 的目标之一。
>
>Mike Belshe, More Bandwidth Doesn't Matter (Much)

上述结论可能会让许多人感到惊讶，但他们应该理解这些，因为它们是底层协议的性能特征的直接后果：TCP 握手，流量和拥塞控制以及由于丢包引起的线头阻塞。大多数 HTTP 数据流由小型突发数据传输组成，而 TCP 是针对长连接和批量数据传输进行优化。大多数情况下，网络往返时间是 TCP 吞吐量和性能的限制因素;请参阅[优化 TCP](https://hpbn.co/building-blocks-of-tcp/#optimizing-for-tcp)。因此，延迟也是 HTTP 和大多数 Web 应用程序交付的性能瓶颈。

>*note*
>
>如果延迟是大多数有线连接性能的限制因素，那么您可能会直观地看到，这对于无线客户端来说是一个更重要的性能瓶颈：无线延迟明显更高这使得网络优化成为移动网络的重中之重。
