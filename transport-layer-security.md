...
> 2. $> openssl speed aes
>
> 需要注意，两个测试的单位不是可以直接比较的，Elliptic Curve Diffie-Hellman (ECDH) 测试提供了每种不同密钥大小所需时间的汇总表，而 AES 的结果通过每秒处理的字节数来衡量。无论如何，ECDH 操作明显更加耗时。
>
...
典型的应用数据传递流程如下：
1. 记录协议收到应用数据
2. 将接受到的数据分块，最大 2^14 字节或者说 16 KB 每个记录
...
1. 最大 TLS 记录是 16 KB
2. 每一个记录包含一个 5 字节的头，一个 MAC (对于 SSLv3，TLS 1.0，TLS 1.1 大约 20 字节，对于 TLS 1.2 大约 32 字节)
3. 解密数据的时候需要整个记录
...
5. 不同服务器应该共享缓存使得会话缓存利用率提升，需要建立一个安全机制来共享并更新密钥来解密会话票据。
6. 检测并监视你 TLS 会话的统计信息，确保性能最高

实践中，你应该配置会话缓存和会话票据机制。这些机制合在一起覆盖了新老连接。

## 启用 TLS False Start
会话重用提供了两个重要的好处：对重新访问的浏览者减少了一个额外的握手以及通过使用之前达成一致的会话参数降低了计算开销。然而，这不能帮助第一次和服务器交流的访问者，或者是之前会话过期的情景。

为了覆盖新老访问者，我们可以使用 TLS False Start，它是一个可选的协议扩展，运行发送者在握手只完成一部分的情况下发送数据，如图 4-10。

![](assets/2017-05-07-10-34-16.png)

图 4-10 启用了 False Start 的 TLS 握手

False Start 不改变 TLS 握手协议，而是只改变了可以发送数据的时间。一旦客户端发送了 *ClientKeyExchange* 记录，它就已经知道了加密密钥并且可以开始传输应用数据，剩下的握手只是用来确认没有人可以伪造了握手记录，这是可以并行执行的。结论是 False Start 保证了无论是完整的 TLS 握手还是简化版的都只有一个往返。

> **部署 TLS False Start**
> 因为  False Start 仅仅改变了握手协议发送应用数据的实际，它不需要更新 TLS 自身，这就意味着可以独立实现，例如客户端可以简单地开始传输加密过的应用数据。
>
> 实际上，即使 TLS False Start 应该和所有现存的 TLS 客户端和服务器向后兼容，因为一些服务器不好的实现，对于所有的连接默认开启 TLS False Start 也会造成问题。结论是，所有的现代浏览器都可以使用 TLS False Star他，但是只有在服务器符合一定条件的情况下才可以使用。
>
> 1. Chrome 和 Firefox 需要握手时服务器提供 ALPN 协议建议，并且服务器选择的加密算法支持前向安全。
> 2. Safari 要求服务器选择的加密算法支持前向安全。
> 3. Internet Explorer 使用了已知不支持 False Start 的黑名单，并且在 TLS False Start 握手失败之后重新进行握手
>
> 为了在所有的浏览器中启用 TLS False Start，服务器应该通过 ALPN 扩展提供支持协议列表。例如 “h2,http/1.1” 并且最好支持并优先使用启用了前向安全的加密算法。

## 优化 TLS 记录大小
通过 TLS 传输的所有数据都通过一个记录协议，如图 4-8。每个记录的最大大小是 16 KB，这取决于选择的加密套件，每一个记录会被加入一个 20 到 40 不等的字节，包含了包头，MAC 和可选的填充数据。如果记录可以放进一个单独的 TCP 包，那么还会添加 IP 和 TCP 数据：IP 协议的 20 字节的包头，没有选项的 TCP 协议需要 20 字节的包头。这就意味着每个记录有大约 60 到 100 字节的额外数据。对于一个典型的最大传输单元（MTU）是 1，500 字节，这个包结构至少造成了 6% 成帧开销。

记录越小，成帧开销越大。然而，简单的增加记录的最大大小并不是一个好主意。如果记录被分散在多个 TCP 包中，那么 TLS 层一定要等待所有的 TCP 包到达才可以开始解密数据，如图 4-11.如果任何一个 TCP 数据包出现因为拥塞控制造成的丢失，重排或超时，那么这个 TLS 记录必须被缓存下来直到可以解码，这就造成了额外的延迟。实际中，这些延迟对于更喜欢流式数据的浏览器造成了显著的瓶颈。

![](assets/2017-05-07-11-02-46.png)

图 4-11 WireShark 捕获的 11,211 字节的 TLS 记录被分割到 8 个 TCP 段

小记录造成了额外开销，大记录造成了额外延迟，没有任何一个值是最优的记录大小。相反，对于浏览器使用的网络应用，最好的策略是根据 TCP 连接动态调整记录大小：
1. 当建立新连接的时候，TCO 拥塞窗口很小，连接会较为空闲一段时间（查看[慢启动](https://hpbn.co/building-blocks-of-tcp/#slow-start-restart)），每一个 TCP 数据包应该恰好传输一个 TLS 记录，TLS 记录应该恰好占据TCP 分配整个数据段大小(MSS)
2. 当拥塞窗口已经很大，我们正在传输一个很大的数据流（例如视频流），TLS 记录的大小应该增加到需要分散到多个 TCP 数据包（大于 16KB）以减少客户端和服务器的成帧和 CPU 开销。

**note**
> 突然发送新数据的时候，如果 TCP 连接是闲置的，甚至服务器禁用了慢启动，最好的策略是降低记录大小。我们的目标是最小化由于丢包，重排已经重发造成的应用层缓存的概率。

使用动态策略传递数据和流量相互作用，实现了最好的性能：小记录大小避免了不必要缓存造成的延迟并且降低了首次接受到第一个-{HTML，视频帧}的时间；大记录大小优化了吞吐量，对于长时间的视频流降低了 TLS 的开销。

为了决定每个状态下的最优记录大小，让我们从一个新连接或者空闲连接开始，这时我们想要避免 TLS 记录分散到多 TCP 包中：
1. IPV4 成帧开销是 20 字节，IPv6 应该是 40 字节
2. TCP 成帧开销是 20 字节
3. TCP 可选项（时间戳，SACK 等）需要 40 字节

假设一个常见的 MTU 是 1,500 字节，这就给通过 IPv4 传递的 TLS 记录剩下了 1,420 字节，IPv6 的话是 1400 字节。在接下来的证明中，我们使用 IPv6 情况下的数据，即留给每一个 TLS 记录 1,400 字节，如果你的 MTU 更低的话，你可以调整它。

通常来说，配置 TLS 记录大小不是我们在应用层可以控制的，相反，它经常是 TLS 服务的编译时常量。检查你服务器的文档来获取更多的细节。

> **Google 的 TLS 优化**
> 在 2014 年早期，谷歌的服务器对于最开始的 1 MB 数据使用了较小的 TLS 记录大小以便放进单个 TCP 段中，之后增加到 16 KB 来优化吞吐量。在一秒钟没有反馈之后重置大小。
>
>同样的，如果你的服务器需要处理大量的 TLS 连接，最小化每一个连接的内存使用量是一个重要的优化。诸如 OpenSSL 等流行的库默认给每一个连接分配 50 KB 的内存，和记录大小一样，这值得你研究文档或源代码来调整这个值。Google 的服务器将这个它的 OpenSSL 缓存降低到大约 5 KB。

## 优化证书链
检查信任链需要浏览器从网站证书开始遍历整个链条，递归验证每一个证书的父节点直到根节点。因此，每一中间证书都提供信任链是很关键的。如果任何一个忽略掉了，浏览器只能被迫暂停验证过程去获取缺失的证书，这引入了额外的 DNS 查找，TCP 握手和 HTTP 请求。

> **note**
> 浏览器如何直到去哪里获取缺失的证书呢？每一个子证书通常包含了一个父证书的 URL。如果省略了这个 URL 并且没有包含所需的证书，这个认证便会失败。

相反的，不要包含必要的证书，例如证书链的根节点-这些信息添加了额外的字节。服务器证书链是被包含在 TLS 握手中，这最可能发生在新的 TCP 连接建立的时候，此时很可能处于慢启动算法的状态。如果证书链大小超过了 TCP 初始拥塞窗口大小，那么我们无意中添加了 TLS 握手的往返：证书长度超过了拥塞窗口造成了服务器暂停，等待客户端的 ACK。

实际上，证书链的大小和深度对于老旧的 TCP 协议栈是更大的问题，它们通常初始化拥塞窗口大小是 4 个 TCP 段，查看[慢启动](https://hpbn.co/building-blocks-of-tcp/#slow-start)。对于新的站点，初始窗口大小增加到了 10 个 TCP 端，这对于大多数的证书链而言已经足够了。
...
1. OCSP 响应大小在 400 到 4000 字节不等。这这个响应包含在证书链中会增加它的大小，时刻注意整个证书链的大小，不要让它超过新 TCP 连接的拥塞窗口大小。
...
## 开启 HTTP 严格传输安全(HSTS)
HTTP 严格传输安全是一个重要的安全机制，它服务器通过一个简单的 HTTP 头声明访问规则，例如"Strict-Transport-Security: max-age=31536000". 特别的，它让客户端遵守如下规则:
1. 所有的请求必须通 HTTPS。这包含了协商和所有同源的子资源请求。例如，一个用户输入了 URL 却没有加上 https 的前缀，那么应该自动的转换成 https 请求。如果一个页面包含了一个非 https 的请求，应该讲其自动转换到 https 版本。
2. 如果一个安全连接不能建立，不能允许用户忽视经过，通过 http 版本请求数据。
3. max-age 指定了 HSTS 规则集的最大存活时间，单位是秒（例如 max-age=3153600 意味着相关建议的生命周期是 365-天）
4. *includeSubdomains* 意味着规则适用于当前站点的所有子域名

HSTS 转换的 HTTPS-only 的目标主机，帮助应用避免了一系列被动和主动的网络攻击。作为一个彩蛋，通过建设 HTTP 到 HTTPS 的转换，它也提供了一个很好的性能优化。客户端自动将所有向安全站点的请求转换为 HTTPS。

**note**
> 在开启 HSTS 之前确保你的 TLS 站点可以通过测试。一旦这个机制被客户端缓存子阿里，TLS 连接协商失败会造成无法恢复的失败-例如 用户会看到浏览器错误界面，无法继续操作。这个行为是一个显式且必要的用来阻止试图通过不用 HTTPS 访问的网络攻击。

> **HSTS 预加载列表**
> HSTS 机制无法阻止第一个请求被主动攻击者攻击。例如，恶意代码可以阻止客户端的完成 HSTS 机制的注册。为了解决这个问题，大多数浏览器提供了 “HSTS 预加载列表”，它允许访问站点的时候包括了预加载列表中的开启了 HSTS 的站点。
>
> 一旦你完成了 HTTPS 的部署，通过 [hstspreload.appspot.com](https://hstspreload.appspot.com/) 提交你的网站给 HSTS 预加载列表

# 开启  HTTP Public Key Pinning （HPKP）
[信任链和证书授权](https://hpbn.co/transport-layer-security-tls/#chain-of-trust-and-certificate-authorities) 中讨论的系统有一个缺点是依赖大量的需要授信的证书认证机构（CA）。换句话说，我们需要从很多的地方获取合法的证书。然而，这也意味着有许多地方可以声明一个合法的证书而不需要源服务器显式的同意。

**note**
> DigiNotar 证书认证机构的危害是一个的引人注目的例子。攻击者能够发行和使用虚假但是有效的证书来伪造数以百计的流行网站。

HTTP Public Key Pinning 允许站点发送一个 HTTP 头让浏览器记住（pin）一个或多个信任链中的证书。这样做可以检查应该被浏览器认可的证书。
1. 源站点不能 “pin” 它的子证书。这是一个重要的安全策略，因为这是将一小部分特定的可以被浏览器接受证书签名硬编码。
2. 源站点可以 “pin” 父证书链中的一个。举个例子，源站点可以 “pin” 到 CA 的一个中间证书，这告诉浏览器只应该接受有特定证书颁发机构的特定的证书签名。

选择合适的策略选择那些证书被 “pin“，如何提供和提供哪些 “pin”，如何部署 HPKH 都是重要又微妙的问题，这超出了我们的讨论范围。用你最喜欢的搜索引擎或者当地的安全专家查询相关信息。
...
## 性能检查列表
对于应用开发者而言，我们封装了大多数 TLS 协议的复杂特点，客户端和服务器只需要在我们的抽象之上工作即可。然而正如我们在本章中已经看到的那样，这不意味着我们可以通过 TLS 传递数据而忽略性能方面的表现。开启服务器中关键的优化项并且配置应用使其可以让客户端发挥它的有事：更快的握手，更低的延迟，更好的安全保证等等。

请将下列简单的列表记在心里：
1. 取得最好的 TCP 性能，查看 [TCP 优化](https://hpbn.co/building-blocks-of-tcp/#optimizing-for-tcp)
2. 升级 TLS 库到最新的版本，重新构建服务器
3. 开启并配置会话缓存和无状态恢复
4. 监视你的会话缓存命中率，并以此调整配置
5. 配置前向安全以开启 TLS  False Start
6. TLS 服务器距用户更近，降低往返时间
7. 使用动态 TLS 记录大小来优化延迟和吞吐
8. 检查并优化证书链的大小
9. 配置 OCSP stapling.
10. 配置 HSTS and HPKP.
11. 配置 CSP
...
检查证书链会发现服务器发送了两个证书，这增加了 3,571 字节。并且我们可以看到 TLS 会话协商变量，选择的协议，加密算法和密钥，我们还可以看到服务器为当前会话其数量会话标识符，这可以在将来重用。