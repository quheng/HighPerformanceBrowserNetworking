...
## 优化 TLS 记录大小
通过 TLS 传输的所有数据都通过一个记录协议，如图 4-8。每个记录的最大大小是 16 KB，这取决于选择的加密套件，每一个记录会被加入一个 20 到 40 不等的比特，包含了包头，MAC 和可选的填充数据。如果记录可以放进一个单独的 TCP 包，那么还会添加 IP 和 TCP 数据：IP 协议的 20 比特的包头，没有选项的 TCP 协议需要 20 比特的包头。这就意味着每个记录有大约 60 到 100 比特的额外数据。对于一个典型的最大传输单元（MTU）是 1，500 比特，这个包结构至少造成了 6% 成帧开销。
...
图 4-11 WireShark 捕获的 11，211 比特的 TLS 记录被分割到 8 个 TCP 段
...
1. IPV4 成帧开销是 20 比特，IPv6 应该是 40 比特
2. TCP 成帧开销是 20 比特
3. TCP 可选项（时间戳，SACK 等）需要 40 比特

假设一个常见的 MTU 是 1,500 比特，这就给通过 IPv4 传递的 TLS 记录剩下了 1,420 比特，IPv6 的话是 1400 比特。在接下来的证明中，我们使用 IPv6 情况下的数据，即留给每一个 TLS 记录 1,400 比特，如果你的 MTU 更低的话，你可以调整它。
...
相反的，不要包含必要的证书，例如证书链的根节点-这些信息添加了额外的比特。服务器证书链是被包含在 TLS 握手中，这最可能发生在新的 TCP 连接建立的时候，此时很可能处于慢启动算法的状态。如果证书链大小超过了 TCP 初始拥塞窗口大小，那么我们无意中添加了 TLS 握手的往返：证书长度超过了拥塞窗口造成了服务器暂停，等待客户端的 ACK。
